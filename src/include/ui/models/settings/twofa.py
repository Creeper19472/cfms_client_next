...
class TwoFactorSettingsModel(Model):
    ...
    async def _load_2fa_status(self):
        ...
        try:
            ...
            if response.code == 200:
                ...
                send_error(self.page, f"Failed to load 2FA status: {response.message}")
                
        except Exception as e:
            send_error(self.page, f"Error loading 2FA status: {str(e)}")
        finally:
    ...
    async def _on_toggle_2fa(self, e):
        ...
        try:
            ...
            send_error(self.page, f"Error setting up 2FA: {str(e)}")
        finally:
            self.loading_ring.visible = False
            self.toggle_button.disabled = False
            self.update()
